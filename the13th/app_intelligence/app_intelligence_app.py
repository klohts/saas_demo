...
class Event(SQLModel, table=True):
    ...
    details: Optional[dict] = ORMField(default=None, sa_column=Column(JSON))
...
# -------- App & DB setup --------
app = FastAPI(title="THE13TH App Intelligence")
...
def create_event(payload: dict, x_sys_api_key: Optional[str] = Header(None)):
    ...
    if not client_id or not action:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Missing client_id or action")
