#!/usr/bin/env python3
"""
deploy_guardian.py — v4.1 (Resilient Slack Edition)
---------------------------------------------------
✅ Syntax/import validation
✅ Auto Git push → Render auto-deploy
✅ Auto-detect Render service ID
✅ Deploy monitor + endpoint verification
✅ Postgres logging
✅ Email notifications
✅ Slack notifications with DNS self-healing + retry
"""

import os, subprocess, requests, json, time, psycopg2, sys, smtplib, socket
...
# ===========================================================
class SlackNotifier:
    """Reliable Slack notifier with DNS fallback and retry handling."""

    def __init__(self, webhook: Optional[str]):
        self.webhook = webhook
        if not self.webhook:
            print("⚠️  No SLACK_WEBHOOK found in .env — Slack notifications disabled.")

    def _ensure_dns(self):
        """Ensure DNS resolution works; if not, apply fallback to Google DNS."""
        try:
            socket.gethostbyname("hooks.slack.com")
        except socket.gaierror:
            print("⚠️  DNS resolution failed. Applying fallback to Google DNS...")
            try:
                with open("/etc/resolv.conf", "w") as f:
                    f.write("nameserver 8.8.8.8\nnameserver 1.1.1.1\n")
                print("✅ DNS fallback applied (8.8.8.8 + 1.1.1.1)")
            except PermissionError:
                print("❌ Unable to write /etc/resolv.conf (need sudo).")

    def send(self, text: str, retries: int = 3):
        """Send a Slack message with auto-retry and DNS healing."""
        if not self.webhook:
        ...
        for attempt in range(1, retries + 1):
            try:
                ...
                if r.status_code == 200:
                    print(f"💬 Slack notified successfully (Attempt {attempt})")
                    return
                else:
                    print(f"⚠️  Slack responded with status {r.status_code}: {r.text}")
        ...
        print("❌ Failed to send Slack message after multiple attempts.")
...
def trigger_git_push():
    ...
    os.system("git add .")
    os.system("git commit -m 'Auto-deploy via Deploy Guardian v4.1' || echo 'No new changes'")
...
if __name__ == "__main__":
    print("🚀 Starting Deploy Guardian v4.1 (Resilient Slack Edition)...\n")

    notifier = SlackNotifier(SLACK_WEBHOOK)
    notifier.send("🚀 Deploy Guardian started checking your app...")
    ...
    if not service_id:
        notifier.send("❌ Deploy Guardian failed to detect Render service ID.")
        sys.exit(1)
    ...
    if success:
        ...
        notifier.send(f"🚀 *Deploy Success* — {message}")
    else:
        log_deploy_result("failed", "Render deploy failed ❌")
        send_email("Render Deploy: FAILED ❌", "Render deploy failed. Check logs.")
        notifier.send("💥 *Render Deploy FAILED!* — Please investigate.")

    print("\n✅ Deploy Guardian v4.1 finished.\n")
